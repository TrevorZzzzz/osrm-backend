...
*   `options` **[Object][2]** Object literal containing parameters for the route query.
    ...
    *   `options.annotations` **([Array][5] | [Boolean][4])** An array with strings of `duration`, `nodes`, `distance`, `weight`, `datasources`, `speed`, `elevation` or boolean for enabling/disabling all (`true` does not include `elevation`). (optional, default `false`)
