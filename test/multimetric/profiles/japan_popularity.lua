...
local function apply_popularity_weight(profile, way, result, data)
  local class = way:get_value_by_key('yamap:popularity_class')
  local multiplier = popularity_multiplier[class] or 1.0
  if result.forward_mode ~= mode.inaccessible and result.forward_speed > 0 then
    result.forward_rate = (result.forward_speed / 3.6) / multiplier
  end
  if result.backward_mode ~= mode.inaccessible and result.backward_speed > 0 then
    result.backward_rate = (result.backward_speed / 3.6) / multiplier
  end
  if result.duration > 0 then
    result.weight = result.duration * multiplier
  end
end
